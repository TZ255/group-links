const ARRBotFunction = async () => {
    const axios = require('axios').default
    const { Bot } = require("grammy");

    const bot = new Bot(process.env.ARR_TOKEN);
    ...
    const API = `https://api.coincap.io/v2/assets/pirate-chain`

    bot.command('start', async ctx => {
        try {
            let txt = `Hi <b>${ctx.chat.first_name}</b>\n\nTo get the live price of <b>Pirate Chain Token (ARRR)</b> use these command <b>/arrr</b> or <b>/price</b>`
    ...
    bot.command(['price', 'arr'], async ctx=> {
        try {
            ...
            await ctx.api.sendMessage(imp.pricelogs, `${username} just checked price of ARRR Token`, {parse_mode: 'HTML'})
        } catch (err) {
