const DYDXBotFunction = async () => {
    const axios = require('axios').default
    const { Bot } = require("grammy");

    const bot = new Bot(process.env.DYDX_TOKEN);
    ...
    const API = `https://api.coincap.io/v2/assets/dydx`

    bot.command('start', async ctx => {
        try {
            let txt = `Hi <b>${ctx.chat.first_name}</b>\n\nTo get the live price of <b>dYdX (DYDX)</b> use these command <b>/dydx</b> or <b>/price</b>`
    ...
    bot.command(['price', 'dydx'], async ctx=> {
        try {
...
module.exports = {DYDXBotFunction}